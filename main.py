...
from recommender import get_similar_movies, get_recommendations_by_mood
import numpy as np
import pandas as pd
import httpx
...
async def search_bundle(
    ...
    try:
        ...
        recs_raw = []
        try:
            recs_raw = get_similar_movies(details.title, n=tfidf_top_n)
        except:
            try:
                recs_raw = get_similar_movies(query, n=tfidf_top_n)
            except:
                recs_raw = []
    except Exception:
...
async def recommend_mood(
    ...
    try:
        recs = get_recommendations_by_mood(text, n=top_n)
    except Exception as e:
